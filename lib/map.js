		...
		initTiles: function(){
			...
			for(var i = 0, len = options.tilesData.length; i < len; i++){
				...
				if(gid > 0){
					...
					var posX = x * tileset.tileWidth;
					var posY = y * tileset.tileHeight;
					var tiledObj = new TiledTile({gid: gid, posX: posX, posY: posY, width: tileset.tileWidth, height: tileset.tileHeight, tileset: tileset});
